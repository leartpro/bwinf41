...
\begin{document}
    ...
    Beim Zusammensetzten der Scheiben zu einem Quader ist zu beachten,
    dass diese nicht zwangsläufig alle von der gleichen Seite des Quaders stammen müssen,
    daher müssen alle Dimensionen des Quaders betrachtet werden.
    Aufgrund der mathematischen Eigenschaften eines Quaders, ist es allerdings ausreichend, wenn nur drei der sechs Seiten betrachtet werden.
    ...
    Wenn dies der Fall ist, wird sie zur Lösungsmenge $O$ hinzugefügt und aus der Menge $S$ entfernt.
    Der Quader wird aktualisiert, indem die entsprechende Seitenlänge reduziert wird.
    Eine Lösung ist gefunden, wenn entweder die Länge, die Höhe oder die Tiefe des Quaders, daher das Volumen, null ist.
    Wenn es eine Lösung mit der aktuellen Scheibe gibt, kann sich der Algorithmus beenden.
    Andernfalls werden die Änderungen rückgängig gemacht, indem die Scheibe wieder zur Menge $S$ hinzugefügt wird
    ...
    \begin{algorithm}[H]
        ...
        \begin{algorithmic}
            \Require Die Länge $l$, Höhe $h$ und Tiefe $d$ des Käsequaders, eine Menge $S$ von Käsescheiben, eine leere Menge $O$
            \Ensure Eine boolesche, Angabe ob die Käsescheiben zu einem vollständigen Käsequader zusammengesetzt werden können,
    ...
    Ob eine Scheibe von einem Quader abgeschnitten werden kann und wenn ja, auf welcher Seite,
    ...
    \subsubsection{Problemstellung}\label{subsubsec:problemstellung}
    ...
    Daher, zu welcher minimalen Anzahl von Quadern sich die gegebene Menge an Scheiben zusammensetzten lässt.

    \subsubsection{Lösungsverfahren}\label{subsubsec:loesungsverfahren}
   ...
   oder, sollte er für keine der Varianten eine Lösung finden, wird nicht davon ausgegangen, dass es keine Lösung gibt,
    sondern es wird die Anzahl der möglichen Quader um eins erhöht und der Algorithmus wird für die neue Menge an Varianten ausgeführt.
    ...
    So ist eine Lösung nach der Erweiterung erst dann gefunden, wenn für alle Varianten einer Kombination eine Lösung gefunden wurden.
    ...
    Die Methode \texttt{find\_dimensions} verwendet eine Schleife,
    ...
    Die Methode \texttt{find\_combinations} verwendet eine rekursive Strategie,
