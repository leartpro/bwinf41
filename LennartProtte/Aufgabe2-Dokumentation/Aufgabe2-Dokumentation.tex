...
\begin{document}
    ...
    \begin{algorithm}
        ...
        \label{alg:cheese2}
        \begin{algorithmic}
            \Function{can\_remove\_slice}{$length, height, depth, slice$}
                \If{($slice_{length} = length$ \land $slice_{height} = height$) \lor ($slice_{length} = height$ \land $slice_{height} = length$)}
                    \State \Return $FRONT$
                \ElsIf
